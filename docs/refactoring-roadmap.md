...
- **2026-06 — `pwa`: manual "check for updates" + footer row.** `usePwaUpdate`
  only checked silently (on register, hourly, on focus) and kept the
  registration private, so an adopter wanting a "check now" affordance couldn't
  trigger a probe and had to hand-roll the whole footer row (spinner, "up to
  date" / "update available" feedback, `aria-live`). Lifted in `checkForUpdate()`
  - a `checking` flag on the hook, and a presentational `CheckForUpdatesItem`
    row that owns the chrome. Severity ~7.
