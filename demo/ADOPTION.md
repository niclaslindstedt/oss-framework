...
| [`vite.config.ts`](./vite.config.ts) (`react` → `preact/compat`)                      | Renders the demo on **Preact** instead of React — a bundler-level swap that takes ~34% off the gzipped bundle. Not a framework requirement; the framework still targets React.      | **Your choice, and it is real plumbing — decide, don't inherit it by accident.** KEEP the five aliases (and `npm install preact`) for the smaller bundle, or delete them and `npm install react react-dom` to run on React. Keep `@types/react` and type-check against it either way — see the root README's "Running on Preact". |
