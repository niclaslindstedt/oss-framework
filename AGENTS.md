...
- **No domain / business naming in `src/`.** The framework names the _mechanism_,
  never an app's _use_ of it. A specific app's vocabulary — `archive`, `note`,
  ...
  for what it _does_: a row flick-off is a `commit` action the caller labels; a
  ...
  guards: lift generic responsibility _in_, never drag the store, domain types,
...
| `expand-framework`         | Growing the framework's capability surface ("expand the framework"). Works the expansion roadmap (`docs/expansion-roadmap.md`) of net-new modules — picks the next pending one by phasing, implements it end to end under the standing rules (zero deps, no domain naming, pure core + components, full wiring checklist), flips its Status row to Landed. One module ≈ one PR. |
