...
| `MonthGrid`/`DatePicker` | `.` and `./calendar`     | ARIA month grid (roving tabindex, `renderDay` marker seam, min/max/disabled) and the floating date field built on it — with quick month/year jump (caption → month grid → twelve-year page).       |
