...
| `useEdgeHover`           | `.` and `./hooks`        | Is a fine pointer resting over this element's band? — reveals a control that costs the pointer nothing at rest.                                                                                                        |
...
| `useSwipeNav`            | `.` and `./hooks`        | Touch swipe that steps one place along an ordered axis (tabs, screens, months) — stands down on sliders, dialogs and sideways scrollers; nests via `data-swipe-ignore`.                                                |
| `useUndoRedoShortcuts`   | `.` and `./hooks`        | Global Cmd/Ctrl+Z · Cmd/Ctrl+Shift+Z / Ctrl+Y bound to a document history.                                                                                                                                             |
| keyboard-target probes   | `.` and `./hooks`        | `isEditableTarget` / `keyboardIsClaimed` / `blurActiveField` — who the keyboard belongs to, before a window-level shortcut acts.                                                                                       |
| tap arithmetic           | `.` and `./hooks`        | `isTap` / `isDoubleTap` and the shared thresholds, for a surface that handles its own pointer stream.                                                                                                                  |
| `useSearchShortcuts`     | `.` and `./hooks`        | Global "open search" wiring: Cmd/Ctrl+K, plus type-to-open on a bare printable key; hands the seed to `SearchModal`'s `initialQuery`.                                                                                  |
...
| clipboard reading        | `.` and `./hooks`        | `readClipboard` / `readDataTransfer` — one look, every flavour, every failure empty; `clipboardLookIsFree` separates a free peek from one the user must answer.                                                        |
...
| `useMeasuredSize`        | `.` and `./hooks`        | ResizeObserver-backed element size (null until first measure) — responsive charts, canvases, virtualised lists.                                                                                                        |
| `useFileDrop`            | `.` and `./hooks`        | Drag-and-drop file intake for the window or an element — depth-counted enter/leave, MIME gating, nested-zone claiming.                                                                                                 |
...
| `useThemeColorMeta`      | `.` and `./theme`        | Keep every `theme-color` meta on the colour the active theme resolves `--page-bg` to, so the browser chrome matches the page.                                                                                          |
...
| `createIdbStore`         | `.` and `./storage`      | Keyed record stores over IndexedDB — what a device keeps. In-line or out-of-line keys, best-effort or strict, one schema per database.                                                                                 |
| bulk-transfer politeness | `.` and `./storage`      | `mapLimit` + `withTransientRetries` — keep a many-file sweep inside a provider's throttle and the browser's connection budget.                                                                                         |
| `createLogStore`         | `.` and `./logging`      | In-app log ring buffer + capture mirror; the storage sink.                                                                                                                                                             |
| `LogViewer`              | `.` and `./logging`      | Live Logs panel over a store (newest first, filter, copy, clear); `useLogs` hook.                                                                                                                                      |
...
| `BottomNav`              | `.` and `./components`   | Thumb-reachable bar of destinations for a phone-shaped app — the `Sidebar`'s counterpart; `stepDirection` reads a move along its order for the screen transition.                                                      |
| `FloatingButton`         | `.` and `./sidebar`      | The draggable, edge-resting FAB the shell floats, reusable on its own.                                                                                                                                                 |
| edge-swipe arithmetic    | `.` and `./sidebar`      | `inEdgeZone` / `classifyEdgeDrag` + the two thresholds, so a surface handling its own pointer stream shares the drawer's rule instead of copying it.                                                                   |
| collapse rails           | `.` and `./sidebar`      | `SidebarCollapseRail` folds the docked panel from a click-through edge rail; `CollapseRail` folds the section below it.                                                                                                |
| `Modal`                  | `.` and `./components`   | Portalled accessible dialog (backdrop, focus trap, scroll lock).                                                                                                                                                       |
| `useDialogDrag`          | `.` and `./components`   | Move a dialog card by its title row, for the dialogs whose position is a real question; `Modal` reads the offset into its `translate`.                                                                                 |
| `SwipeDeck`              | `.` and `./components`   | A pager: a track of three panes, a drag that follows the finger, a settle that commits. Either axis, nestable perpendicular, scrolling panes.                                                                          |
| `IconButton`             | `.` and `./components`   | The 36 px square glyph button, with the ARIA a hand-rolled one gets wrong — `aria-pressed` for a toggle, `aria-haspopup`/`aria-expanded` for a disclosure.                                                             |
| plain-text editable      | `.` and `./components`   | Read, write, and seat the caret in a `contenteditable` box — the writing surface for text that has to wrap around a float or shrink to fit.                                                                            |
...
| `ActionMenuList`         | `.` and `./components`   | The menu body on its own — the keyboard-navigable `role="menu"` the two gesture wrappers share, for a menu your own trigger opens.                                                                                     |
...
| `Avatar`                 | `.` and `./components`   | Entity avatar disc — image → icon → initials → fallback cascade across five named sizes, tinted via `currentColor`.                                                                                                    |
| labeled fields           | `.` and `./components`   | `LabeledInput` / `LabeledTextarea` — a caption over a bordered field holding a local draft, committing on blur/Enter.                                                                                                  |
| toast stack              | `.` and `./components`   | `createToastStore` + `ToastViewport` — capped, pausable, `aria-live` notification stack with an action (undo) seam.                                                                                                    |
...
| `ActionPill`             | `.` and `./components`   | Floating pill of two or three verbs, portalled and centred over the content they act on.                                                                                                                               |
| `AnchoredFlash`          | `.` and `./components`   | Portalled "Copied" label that flicks up over the value it happened to.                                                                                                                                                 |
| `ReorderButtons`         | `.` and `./components`   | Tight up/down chevron pair for hand-ordering rows; `LabeledDateInput` is the iOS-safe uncontrolled date field.                                                                                                         |
...
| chart components         | `.` and `./charts`       | `LineChart`, `BarChart`, `DonutChart`, `Sparkline` — zero-dep SVG charts painted with the theme's token palette.                                                                                                       |
| chart math               | `.` and `./charts`       | Pure scales/ticks/paths: `linearScale`, `timeScale`, `bandScale`, `linearTicks`, `niceTicks` (values + label decimals under a tick cap), `timeTicks`, `stackSeries`, `donutArcs`, …                                    |
| `zip` module             | `.` and `./zip`          | Dependency-free ZIP read/write (`createZip`, `readZip`) over `CompressionStream`, with CRC-32 and a stored-entry fallback.                                                                                             |
| calendar math + ICS      | `.` and `./calendar`     | Recurring-date math (year-optional parsing, `yearsSince`, `nextOccurrence`) + iCalendar `.ics` serialization.                                                                                                          |
| day-grid core            | `.` and `./calendar`     | `DayKey` day identity + pure grid math: `addDays`/`addMonths` (clamped), `startOfWeek`, `isoWeek`, `buildMonthGrid`, `buildWeekStrip`, day ranges.                                                                     |
| `MonthGrid`/`DatePicker` | `.` and `./calendar`     | ARIA month grid (roving tabindex, `renderDay` marker seam, min/max/disabled) and the floating date field built on it — a form's date input without a native picker, with quick month/year jump and an `invalid` state. |
| `MonthCalendar`          | `.` and `./calendar`     | `MonthGrid` that pages: month heading, prev/next arrows, PageUp/PageDown and a swipe, over a cursor it owns. Labels injected.                                                                                          |
| `useDayPress`            | `.` and `./calendar`     | Press-and-hold on a day cell, added from outside the grid — reads the day off `MonthGrid`'s `data-day`, swallows the click, cancels the callout.                                                                       |
| `DayKey` rendering       | `.` and `./calendar`     | `dayKeyToDate` (local midnight, never UTC-shifted), `formatDayKey`, `formatMonthLabel`.                                                                                                                                |
| observance rules         | `.` and `./calendar`     | The arithmetic a holiday table is written in: `easterSunday` (Gregorian computus), `addToMonthDay`, `weekdayOnOrAfter`, `nthWeekdayOfMonth`, `lastWeekdayOfMonth`.                                                     |
| file plumbing            | `.` and `./files`        | Save/open Blobs and data-URL payloads, FileReader intake with a size cap, base64 data-URL ⇄ bytes codec.                                                                                                               |
| format helpers           | `.` and `./format`       | Pure formatting: URL tidying, digit-grouping primitives, `formatBytes` over cached `Intl.NumberFormat`.                                                                                                                |
| `Intl` wrappers          | `.` and `./format`       | `formatNumber`, `formatCompact`, `formatDate`, `formatRelative`, `formatDuration`, `weekdayNames`, `weekdayOrder`, `monthName` — cached, locale-as-parameter.                                                          |
| phone + postal           | `.` and `./format`       | `parsePhone` / `toStoredPhone` / `phoneDialString`, and `addressLines` / `formatAddress` / `mapsUrl` / `parseAddress`.                                                                                                 |
| media viewer             | `.` and `./viewer`       | `Lightbox` (full-screen swipe-paging overlay), `ImageCropper` (circular-mask pan/zoom crop dialog), `usePanZoom` + pure framing math (`zoomAboutPoint`, `clampTransform`) and canvas bake helpers.                     |
| expression core          | `.` and `./expression`   | Infix arithmetic as text: `evaluate` / `isEvaluable` / `closeParens`, `expressionSegments` + `toggleSign`, `chainExpression`, `pasteCandidate`.                                                                        |
| `ExpressionText`         | `.` and `./expression`   | An expression set as it reads — operator chips, symbol functions, bracket colouring; `RevealText` types the same thing in.                                                                                             |
| text fitting             | `.` and `./fit`          | Text that fits its box: a pre-layout size band (`resolveFontPx`, `scaleBand`) and the measured pass (`fitTextSize`, `clipTextToBox`) that says whether even the floor was too big.                                     |
