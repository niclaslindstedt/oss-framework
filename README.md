...
| calendar math + ICS      | `.` and `./calendar`     | Recurring-date math (year-optional parsing, `yearsSince`, `nextOccurrence`) + iCalendar `.ics` serialization — slice 1.                                                                            |
...
| format helpers           | `.` and `./format`       | Pure formatting (slice 1): URL tidying, digit-grouping primitives, `formatBytes` over cached `Intl.NumberFormat`.                                                                                  |
