...
// no DOM, no I/O — so it is cheap to unit-test. The changelog renderer
// (`render.tsx`) turns the nodes this produces into React.
//
// `classifyLines` assigns each source line a block kind (heading, list,
// quote, …); `parseInline` tokenises a line's text into inline nodes (bold,
// italic, code, links, …). It is intentionally a pragmatic subset of
// CommonMark — it covers the constructs changelog bullets and feature docs
// reach for and favours predictable behaviour over completeness.
 ...
 * so every leaf node's `offset` is absolute within the original line.
 */
